<template>
  <div class="space-y-4">
    <div v-if="props.isLoading" class="text-center py-10 text-gray-500">
      Loading organizations...
    </div>
    <div v-else-if="props.error" class="text-center text-red-500">{{ error }}</div>
    <div
      v-else-if="!props.isLoading && props.organizations.length === 0"
        ...
        <tbody>
          ...
          >
            ...
            <td class="px-4 py-2 space-x-2">
              <button @click="editOrganization(organization)" class="text-blue-600 hover:underline">
                แก้ไข
              </button>
              <button @click="confirmDelete(organization)" class="text-red-600 hover:underline">
                ลบ
              </button>
            </td>
          </tr>
